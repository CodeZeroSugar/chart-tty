...
func TestValidateBracketContent(t *testing.T) {
	tests := []struct {
		...
		{"german H root accepted", "H", true},
		{"german H seventh accepted", "H7", true},
		{"relaxed-mode chord accepted", "Coda", true},
		{"relaxed-mode star extension accepted", "Gm*", true},
		{"relaxed-mode section word accepted", "Chorus", true},
