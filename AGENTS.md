...
- **M8 — AI-driven chart converter**: `internal/aichart` — takes most existing chord charts (messy, non-compliant) and reformats them to fully ChordPro-compliant output.
  ...
  - Conversion loop: build prompt (raw chart + ChordPro rule summary, "output only compliant ChordPro") → call model → run the existing `Validator.ValidateChart` on the output → on failure feed the validation errors back and retry (bounded, ~3 attempts) → surface original + errors if still failing. The system prompt embeds `internal/parser/chordpro-spec.md` sections via `promptSpecCore`.
  - Triggers: `--ai-convert` CLI flag (`--write` emits `<name>.pro` next to source) and a TUI keybind (`c`) converting the loaded chart in-session.
...
- **M9 — SQLite library**: `internal/db` over `modernc.org/sqlite` — charts stored as blobs with extracted title/artist/source; setlists as ordered items. Migrations via `schema_version`.
- **M10 — TUI screens**: screen enum on the Model (`viewChart`, `browseCharts`, `browseSetlists`, `pickSetlist`, `viewSetlist`); `L`/`S` entry keys, esc back, enter select.
- **M11 — Import + save**: `i` imports the loaded chart, `s` saves an AI conversion (converted content only, source `ai`), `--import <file>` CLI flag.
- **M12 — Setlists**: create via typed name (`bubbles/textinput`), add browsed chart from library browser, sequential view where paging past a chart's last page advances to the next setlist chart (and back-paging returns to the previous one).
- **M13 — Polish**: per-screen help lines, README coverage for library/setlists/config `[library]`.