...
- Bubbletea: raw `Key` constants (`tea.KeyEsc`) are NOT `KeyMsg` and silently fall through a `case tea.KeyMsg:` type switch — always wrap: `tea.KeyMsg{Type: tea.KeyEsc}`. Symptom: a keybinding "does nothing" only in tests.
- Never pipe `go test` into `tail`/`grep` inside `&&` chains — the pipe masks go test's exit status and red suites can slip into commits. Use `go test ./... -count=1 && ...` bare.
