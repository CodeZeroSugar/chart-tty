...
- Debug `println` probes in bubbletea code print straight into the rendered UI once the inline renderer degrades. Grep for `println(` before every commit that touched UI code; route real diagnostics to stderr or a log file.- Layout mode decisions must have a single source of truth (`Model.useColumns`) consumed by both View and clampOffset — deriving them independently lets a wide-but-short chart end up clamped for 2H rows while rendering H.
