...
- Multi-minute operations need visible progress or they look hung — `ConvertProgress` emits milestone events consumed by a braille spinner (TTY stderr) or plain lines (piped). In bubbletea, long-running work must run in a `tea.Cmd` goroutine returning a msg; blocking inside `Update` freezes the whole UI.
- Two-column layout lives in the viewport layer (`ui/layoutColumns`), not the renderer — column count depends on terminal size, which the renderer must not know about. Soft-wrap rendered lines into display rows *before* chunking so nothing is lost. Body height must subtract chrome rows (title/message/help) or content overflows the terminal edge.
- When a hand-computed test expectation disagrees with obviously-correct code, re-derive from the actual input bytes before suspecting the code — off-by-one transcription errors in `want:` values are common and the failure output is easy to misread.
- **Never measure ANSI-styled strings with `len()` or rune counts** — lipgloss-wrapped lines (section headers, comments) carry invisible escape sequences that inflate the count and silently break column alignment. Measure with `reflow/ansi.PrintableRuneWidth` (already a lipgloss dependency). Note: `muesli/reflow/ansi` has `PrintableRuneWidth` but no `Strip`; write a small CSI scanner if stripping is needed.
- Tab gate (locked rule): a `{sot}` block is tablature only if ≥4 lines match `^[A-Ga-g][ \t]*\|`. Implemented as a buffer on the Parser — content lines buffer while `activeEnv == "tab"`, and the buffer is evaluated on every exit from the env (`{eot}`, unrelated start/end, EOF). Forgetting any exit path leaks stale buffers into later blocks.
- Spec-alignment rulings (2026-08): attribute/bare directive argument forms parse; `x_*` ignored silently; conditional selectors stripped; spec-legal unsupported directives (define/transpose/delegated/output/legacy fonts) validate-ok + skip at parse via `ignored_directives` in spec.json; generic `start_of_*`/`end_of_*` names are environments; German `H`/`Hb` roots added; **relaxed chord mode** accepted (root + any non-pipe tail — `[Coda]`, `[Chorus]`). Relaxed tails must exclude `|`: otherwise TAB lines like `E|--0--|` validate as chords and corrupt basic-format chord-line detection.
- The OpenCode Go gateway fails/hangs on large system prompts: keep the AI system prompt at or below ~100 lines / ~2.5K prompt tokens with full charts as user input. `promptSpecCore` extracts the operative spec sections for this reason; the full doc stays canonical.
- Bubbletea: raw `Key` constants (`tea.KeyEsc`) are NOT `KeyMsg` and silently fall through a `case tea.KeyMsg:` type switch — always wrap: `tea.KeyMsg{Type: tea.KeyEsc}`. Symptom: a keybinding "does nothing" only in tests. Related trap: changing a helper's parameter from `string` to `tea.Msg` while the caller still passes the string makes every keypress vanish at a failed type assertion — when keys "do nothing" everywhere, check the call-site argument type first.
- Never pipe `go test` into `tail`/`grep` inside `&&` chains — the pipe masks go test's exit status and red suites can slip into commits. Use `go test ./... -count=1 && ...` bare, or check PIPESTATUS.
- Bubbletea inline mode (default `tea.NewProgram(m)`) repaints *below* prior output and cannot clear lines above — frames taller than the terminal accumulate garbage and, past the renderer's line budget, it dumps raw output. Multi-screen TUIs must use `tea.WithAltScreen()`, and every screen's View should emit at most `height` lines.
- Debug `println` probes in bubbletea code print straight into the rendered UI once the inline renderer degrades. Grep for `println(` before every commit that touched UI code; route real diagnostics to stderr or a log file.- Every path that loads a chart into the TUI viewer must set `Model.rawChart` — `startConversion` (the `c` key), the `m` mode toggle's re-parse, and `i` import all read it. A chart opened from the library or a setlist silently had it empty, producing the misleading "AI conversion not configured"; disk opens set it. New open paths must remember it.
- The TUI conversion must stream progress, not just show a static "converting…": wire `ConvertProgress` into a mutex-guarded holder and refresh the corner message with a `tea.Tick` pump (`tea.Batch(convertCmd, tickCmd)`). Note `tea.BatchMsg` is `[]Cmd` (sub-commands), not `[]Msg` — tests must execute each sub-command and feed the results, not treat them as messages.
- Bubbletea does not synthesize a `WindowSizeMsg` for non-terminal outputs (e.g. `tea.WithOutput(bytes.Buffer)` in tests), leaving `Model.width == 0` — and `headerRow` truncates the corner message to "…" at width 0. Program-level rendering tests must `p.Send(tea.WindowSizeMsg{Width:…, Height:…})` before asserting on rendered text, or the assertion silently fails.
- Layout mode decisions must have a single source of truth (`Model.useColumns`) consumed by both View and clampOffset — deriving them independently lets a wide-but-short chart end up clamped for 2H rows while rendering H.
